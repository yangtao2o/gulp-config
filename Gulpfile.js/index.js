...
function typescript() {
  return tsProject.src().pipe(tsProject()).js.pipe(dest(config.output.ts));
}
...
function devServe(cb) {
  ...
  watch(config.input.src).on('change', (path, stats) => console.log(`File ${path} was changed`));
  watch(config.input.src).on('add', (path, stats) => console.log(`File ${path} was added`));
  watch(config.input.src).on('unlink', (path, stats) => console.log(`File ${path} was removed`));
